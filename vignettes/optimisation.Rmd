...
The function `optimi_stics()` can be used to optimize one or several parameters at a time using one or several output variables for one or several USMs.
...
### Single USM, single output variable

The parameter `data.frame` is used as an argument to `optimi_stics()`. Here is an example of the optimization of the two parameters related to plant height for a single USM: 
...
### Single USM, several output variables

We can optimize the parameter values over several output variables also, by giving them as a vector: 
